...
<form action="<?php echo BASE_URL ?>/product/update_product/<?php echo $pro['id_product']?>" method="POST" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <textarea name="desc_product" style="resize: none;" row="5" class="form-control"> <?php echo $pro['desc_product']?></textarea>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    <label for="pwd">Sách bán chạy</label>
    <select class="form-control" name="product_banchay" id="">
     <?php
      if($pro['product_banchay'] == 0){
     ?>
    <option selected value ="0">Không</option>
    <option value ="1">Có</option>
    <?php
